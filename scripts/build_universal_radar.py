...
- each feed either fetches public/raw observations or is marked failed
...
def max_age_hours_for_feed(feed: dict[str, Any]) -> float:
    if feed.get("max_age_hours") is not None:
        return float(feed["max_age_hours"])

    cadence = str(feed.get("cadence") or "").lower()
    domain = str(feed.get("domain") or "").lower()

    # Daily markets: allows weekends/holidays, rejects month-old stale bars.
    if cadence == "daily_market" or domain == "market_indices":
        return 120.0

    # Near-real-time grid/load feeds.
    if domain == "electrical_grid":
        return 48.0

    # NOAA/SWPC products should normally be recent.
    if domain == "space_weather":
        return 48.0

    # USGS all-month feed should normally have recent events.
...
def assert_fresh_feed(feed: dict[str, Any], latest_ts: pd.Timestamp) -> float:
    ...
    age_hours = max(
        0.0,
        (pd.Timestamp(now_utc()) - latest_ts).total_seconds() / 3600.0,
    )

    max_age = max_age_hours_for_feed(feed)

    if age_hours > max_age:
        raise BuildError(
            f"STALE feed: latest observation age {age_hours:.1f}h exceeds "
...
def adapter_ieso_demand_csv(feed: dict[str, Any]) -> FetchResult:
    ...
    candidates = feed.get("value_column_candidates") or [
        "Ontario Demand",
        "Market Demand",
        "Demand",
    ]
...
def adapter_nyiso_pal_integrated_csv(feed: dict[str, Any]) -> FetchResult:
    ...
    grouped = (
        all_rows.groupby("ts", as_index=False)["value"]
        .sum()
        .sort_values("ts")
    )

    if grouped.empty:
        raise BuildError("NYISO rows fetched, but grouped system load is empty")
...
def adapter_usgs_earthquake_geojson(feed: dict[str, Any]) -> FetchResult:
    ...
    g = df.groupby("ts", as_index=False).agg(
        count=("count", "sum"),
        max_mag=("max_mag", "max"),
        energy_proxy=("energy_proxy", "sum"),
    )

    g["value"] = np.log10(g["energy_proxy"].astype(float) + 1.0) + 0.25 * g[
        "count"
    ].astype(float)
...
def json_table_to_frame(data: Any) -> pd.DataFrame:
    ...
    SWPC endpoints are not uniform:
    - some return array tables: [[header...], [row...]]
    - some return object rows: [{"time_tag": ..., "Kp": ...}]
    """
...
def adapter_noaa_swpc_solar_wind_plasma(feed: dict[str, Any]) -> FetchResult:
    ...
    if out.empty:
        raise BuildError("NOAA plasma JSON parsed but produced no numeric observations")
...
def adapter_noaa_swpc_planetary_kp(feed: dict[str, Any]) -> FetchResult:
    ...
    if out.empty:
        raise BuildError("NOAA Kp JSON parsed but produced no numeric observations")
...
def adapter_yahoo_chart_index(feed: dict[str, Any]) -> FetchResult:
    ...
    Uses quote.close only, not adjusted close.
    Same public vendor endpoint; no synthetic fallback.
    Range retry is used because range=max can return stale series for some symbols
    while shorter ranges can be current.
    """
    ...
    interval = str(feed.get("interval") or "1d").strip()
    configured_range = str(feed.get("range") or "max").strip()

    ranges: list[str] = []
    for r in [configured_range, "10y", "5y", "2y", "1y", "6mo"]:
        if r and r not in ranges:
            ranges.append(r)

    encoded = requests.utils.quote(symbol, safe="")
    candidates: list[tuple[pd.Timestamp, FetchResult, float]] = []
    failures: list[str] = []
    ...
    for period in ranges:
        ...
        try:
            ...
            if errors:
                failures.append(f"{period}: {errors}")
                continue

            results = chart.get("result") or []

            if not results:
                failures.append(f"{period}: no chart result")
                continue
            ...
            if not timestamps or not closes:
                failures.append(f"{period}: missing timestamps/close")
                continue
            ...
            out = (
                pd.DataFrame({"ts": ts, "value": vals})
                .dropna()
                .sort_values("ts")
            )

            if out.empty:
                failures.append(f"{period}: no numeric close observations")
                continue

            latest_ts = out["ts"].iloc[-1]
            ...
            fetched = FetchResult(feed, out, url, int(n))
            candidates.append((latest_ts, fetched, age_hours))

            if age_hours <= max_age_hours_for_feed(feed):
                return fetched

            failures.append(
                f"{period}: stale latest={latest_ts.isoformat()} age={age_hours:.1f}h"
            )

        except Exception as e:
            failures.append(f"{period}: {e}")
    ...
    raise BuildError(
        f"Yahoo chart produced no usable close series for {symbol}. "
...
def fmt_num(x: Any, ndigits: int = 1, default: str = "NA") -> str:
    xf = safe_num(x)
...
def rolling_percent_rank(s: pd.Series, window: int) -> pd.Series:
    ...
    min_periods = max(5, min(window // 3, 20))
...
def rolling_corr(a: pd.Series, b: pd.Series, window: int) -> pd.Series:
    return (
        a.rolling(window=window, min_periods=max(10, window // 3))
        .corr(b)
...
def resample_series(
    ...
    if agg.empty:
        raise BuildError("No observations after resampling")

    return agg


def choose_analysis_signal(raw: pd.Series, feed: dict[str, Any] | None = None) -> pd.Series:
    """Choose analysis transform.

    Market indices should always use log close. That avoids scale-dependent
    residual geometry across SPX/CAC/Nikkei/etc.
    """

    x = raw.astype(float)
    domain = str((feed or {}).get("domain") or "").lower()

    if domain == "market_indices":
        min_val = float(x.min())
        if min_val <= 0:
            raise BuildError("Market index close contains non-positive value; cannot log-transform")
        return np.log(x)

    if (
        x.min() >= 0
        and x.quantile(0.95) > 0
        and (x.quantile(0.99) / max(x.quantile(0.50), 1e-9)) > 5
    ):
        return np.log1p(x)
...
def rolling_lambda_q(y: pd.Series, spans: list[int]) -> tuple[pd.Series, pd.Series]:
    """Estimate retained coherence scale robustly.

    Avoids DataFrame.idxmax(axis=1), which fails on early all-NaN rows.
    """
    ...
    for i, row in enumerate(arr):
        ...
        if finite.any():
            ...
            local_j = int(np.argmax(finite_values))
            original_j = int(np.flatnonzero(finite)[local_j])
            best_span_vals[i] = span_arr[original_j]
            best_corr_vals[i] = row[original_j]

    fallback_span = float(np.median(span_arr))

    best_span = (
        pd.Series(best_span_vals, index=y.index)
        .ffill()
        .bfill()
        .fillna(fallback_span)
    )
...
def phase_state(
    ...
    if (
        (reach < 50 and dust > 60)
        or (d_reach < -10 and dust > 58)
        or (scale_agreement >= 67 and dust > 58)
    ):
        return "FRAGILE"
...
def make_narrative(
    ...
    r = safe_num(current.get("reach"))
    d = safe_num(current.get("dust"))
    dr = safe_num(current.get("d_reach"))
    acc = safe_num(current.get("dd_reach"))
    risk = safe_num(current.get("risk"))
    confirm = safe_num(current.get("scale_agreement"))
    ...
    if state == "STABLE":
        ...
        lead = (
            "Watch phase: either dust is rising, reach is shrinking, "
            "or multi-scale confirmation is appearing."
        )
    elif state == "FRAGILE":
    ...
    scale_note = ""
    confirming = [s for s in scales if s.get("confirming")]

    if confirming:
        windows = ", ".join(str(s.get("window")) for s in confirming)
        scale_note = f" Confirming deterioration windows: {windows}."
    elif scales:
        scale_note = " No multi-scale reach-down/dust-up confirmation yet."

    return (
        f"{label}: {state}. {lead} "
        f"Reach {fmt_num(r)}: {reach_text}; "
        f"dust {fmt_num(d)}: {dust_text}; "
        f"dReach {fmt_num(dr, 2)}, ddReach {fmt_num(acc, 2)}: {velocity_text}; "
        f"risk {fmt_num(risk)}; scale confirmation {fmt_num(confirm, 0)}%."
        f"{scale_note} "
        f"Math components: coherence {fmt_num(comp.get('coherence'), 0)}, "
        f"persistence {fmt_num(comp.get('persistence'), 0)}, "
        f"residual pressure {fmt_num(comp.get('residual_pressure'), 0)}, "
        f"volatility pressure {fmt_num(comp.get('volatility_pressure'), 0)}, "
        f"reversal pressure {fmt_num(comp.get('reversal_pressure'), 0)}, "
        f"innovation pressure {fmt_num(comp.get('innovation_pressure'), 0)}. "
        f"Latest raw observation age {fmt_num(age_hours, 1)}h. "
        f"Structural diagnostic only; not a forecast or operating instruction."
    )
...
def compute_diagnostics(result: FetchResult, analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    raw = resample_series(
        ...
        resample_rule,
        fill_zero=fill_zero,
        limit=int(analysis.get("resample_limit", 6)),
    )
    ...
    age_hours = assert_fresh_feed(feed, latest_ts)

    y = choose_analysis_signal(raw, feed)
    ...
    resid = y - ridge_y
    resid_std = (
        resid.rolling(window=min(max(20, len(y) // 10), 250), min_periods=10)
        .std()
        .replace(0, np.nan)
    )
    ...
    rank_window = min(250, max(30, len(y) // 2))

    vol_pct = rolling_percent_rank(vol, rank_window).fillna(50)
    abs_resid_pct = rolling_percent_rank(abs_dust_z, rank_window).fillna(50)

    signs = np.sign(dy.fillna(0.0))
    reversal = (signs * signs.shift(1) < 0).astype(float)

    reversal_rate = (
        reversal.rolling(window=min(30, max(8, len(y) // 10)), min_periods=5).mean()
        * 100.0
    )

    reversal_pressure = rolling_percent_rank(reversal_rate, rank_window).fillna(50)

    innov = dy / vol.replace(0, np.nan)
    innov_pressure = rolling_percent_rank(innov.abs(), rank_window).fillna(50)

    dust_index = pd.Series(
        safe_clip(
            0.45 * abs_resid_pct
            + 0.25 * vol_pct
            + 0.20 * reversal_pressure
            + 0.10 * innov_pressure
        ),
        index=y.index,
    )
    ...
    lambda_flicker = rolling_percent_rank(
        lambda_q.diff().abs().rolling(12, min_periods=4).mean(),
        rank_window,
    ).fillna(50)

    coherence = rolling_percent_rank(lambda_corr, rank_window).fillna(50)

    same_dir = (
        (np.sign(dy) == np.sign(ridge_y.diff()))
        .astype(float)
        .rolling(30, min_periods=8)
        .mean()
        * 100.0
    )
    ...
    deterioration_pressure = rolling_percent_rank(reach_decay, rank_window).fillna(50)
    ...
    for w in scale_windows:
        ...
        d_recent = dust_index.tail(w)

        if len(r_recent) < 5:
            continue

        mid_idx = max(0, len(r_recent) // 2 - 1)

        rv = float(r_recent.iloc[-1] - r_recent.iloc[mid_idx])
        dv = float(d_recent.iloc[-1] - d_recent.iloc[mid_idx])
    ...
    if scale_rows:
        scale_agreement = (
            100.0 * sum(1 for r in scale_rows if r["confirming"]) / len(scale_rows)
        )
    else:
        scale_agreement = 0.0
    ...
    current = {
        ...
        "lambda_flicker": float(lambda_flicker.iloc[-1])
        if pd.notna(lambda_flicker.iloc[-1])
        else None,
        "scale_agreement": float(scale_agreement),
        "components": {
            "coherence": float(coherence.iloc[-1])
            if pd.notna(coherence.iloc[-1])
            else None,
            "persistence": float(persistence.iloc[-1])
            if pd.notna(persistence.iloc[-1])
            else None,
            "anti_dust": float((100.0 - dust_index).iloc[-1]),
            "residual_pressure": float(abs_resid_pct.iloc[-1])
            if pd.notna(abs_resid_pct.iloc[-1])
            else None,
            "volatility_pressure": float(vol_pct.iloc[-1])
            if pd.notna(vol_pct.iloc[-1])
            else None,
            "reversal_pressure": float(reversal_pressure.iloc[-1])
            if pd.notna(reversal_pressure.iloc[-1])
            else None,
            "innovation_pressure": float(innov_pressure.iloc[-1])
            if pd.notna(innov_pressure.iloc[-1])
            else None,
            "deterioration_pressure": float(deterioration_pressure.iloc[-1])
            if pd.notna(deterioration_pressure.iloc[-1])
            else None,
        },
    }
    ...
    series_df = pd.DataFrame(
        {
            ...
            "dust": dust_index.reindex(raw.index).values,
            ...
            phase_tail.append(
                {
                    ...
                    "dust": item["dust"],
                    "risk": item["risk"],
                    "d_reach": item.get("d_reach"),
                    "state": phase_state(
                        float(item["reach"]),
                        float(item["dust"]),
                        float(item.get("d_reach") or 0.0),
                        0.0,
                    ),
                }
            )
    ...
    for k, v in current.items():
        if isinstance(v, float):
            rounded_current[k] = finite_round(v, 3)
        else:
            rounded_current[k] = v
    ...
    return {
        ...
        "short_label": feed.get("short_label", feed.get("label", feed["key"])),
        ...
        "source_tier": feed.get("source_tier", "unknown"),
        "notes": feed.get("notes", ""),
        "raw_rows": result.raw_rows,
        "points": int(len(raw)),
        "latest_ts": iso(latest_ts),
        "age_hours": round(age_hours, 2),
        "max_age_hours": max_age_hours_for_feed(feed),
...
def build(config_path: Path) -> dict[str, Any]:
    ...
    for feed in feeds:
        ...
        try:
            ...
            print(
                f"OK {key}: {diag['points']} points, "
                f"state={diag['current']['state']}, latest={diag['latest_ts']}, "
                f"age={diag['age_hours']}h"
            )
    ...
    if len(built) < min_success:
        raise BuildError(
            f"Only {len(built)} feeds succeeded; minimum is {min_success}. "
            f"Failures: {failed}"
        )
    ...
    global_state = (
        max(states, key=lambda s: state_rank.get(str(s), -1)) if states else "NO_DATA"
    )

    leaders = sorted(
        built,
        key=lambda b: b["current"].get("risk") or 0,
        reverse=True,
    )[:8]
    ...
    payload = {
        ...
        "mode": "raw live public sources only",
        "policy": {
            "no_synthetic_data": True,
            "no_demo_fallback": True,
            "source_rule": (
                "raw public observations only; market indices use public close feeds; "
                "derived diagnostics are computed after fetch"
            ),
        },
        "analysis": analysis,
        "global": {
            ...
            "states": {
                s: states.count(s)
                for s in ["STABLE", "WATCH", "FRAGILE", "CRITICAL"]
            },
            "median_reach": finite_round(
                np.nanmedian([b["current"].get("reach") for b in built]),
                2,
            ),
            "median_dust": finite_round(
                np.nanmedian([b["current"].get("dust") for b in built]),
                2,
            ),
            "median_risk": finite_round(
                np.nanmedian([b["current"].get("risk") for b in built]),
                2,
            ),
            "leaders": [
                {
                    ...
                    "risk": b["current"].get("risk"),
                    "reach": b["current"].get("reach"),
                    "dust": b["current"].get("dust"),
...
def main() -> int:
    ...
    try:
        ...
        out_path = ROOT / str(
            cfg.get("output", "data/derived/universal_ridge_radar.json")
        )
